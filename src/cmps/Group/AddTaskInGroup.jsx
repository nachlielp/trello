...
export function AddTaskInGroup({ groupId, closeAddTask, addTask, addToTop }) {
    const group = useSelector(state => state.boardModule.board.groups.find(group => group.id === groupId));
    ...
    useEffect(() => {
        if (group.tasks?.length > 0) {
            const groupMaxPos = group.tasks.filter(task => !task.closed).reduce((max, task) => Math.max(max, task.pos), 0);
            const groupMinPos = group.tasks.filter(task => !task.closed).reduce((min, task) => Math.min(min, task.pos), 0);
